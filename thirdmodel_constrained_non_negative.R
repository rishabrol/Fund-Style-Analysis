...
process_fund_data <- function(filepath, ticker) {
  ...
  # log return
  data <- data %>%
    mutate(logret = c(NA, 100 * diff(log(Adj.Close)))) %>%
...
# 5 funds
process_fund_data("iShares Core S&P Total Market ETF (ITOT).xlsx", "ITOT")
process_fund_data("iShares Russell 1000 Growth ETF (IWF).xlsx", "IWF")
process_fund_data("T. Rowe Price Small- Cap Value Fund (PRSVX).xlsx", "PRSVX")
process_fund_data("Vanguard Wellesley Income Fund (VWINX).xlsx", "VWINX")
process_fund_data("Vanguard 500 Index Fund (VFIAX).xlsx", "VFIAX")
# risk free
rf_data <- read.csv("Rf_daily_2010_2024.csv") %>%
  select(Date, RF) %>%
...
# 2*3 style
style_value_weight <- read.csv("Average_Value_Weighted_Returns.csv")
...
long_term_bond <- read.csv("Long_term_govbond_log_return_daily.csv", stringsAsFactors = FALSE)
...
Dmat <- t(X_mat) %*% X_mat
dvec <- t(X_mat) %*% Y_vec
Amat <- cbind(rep(1, ncol(X_mat)), diag(ncol(X_mat)))
bvec <- c(1, rep(0, ncol(X_mat)))
meq <- 1
...
##### plot 5 funds#####
# 6 betas plot
##### plot exposures for 5 funds (7 factors including Bond) #####
tickers <- c("ITOT", "IWF", "PRSVX", "VWINX", "VFIAX")
...
for (ticker in tickers) {
  ...
  # Plot exposures including Bond factor
  p <- ggplot(rolling_factors_long, aes(x = Year, y = Beta, color = Factor)) +
    geom_line(size = 1) +
    geom_point(size = 2) +
    ggtitle(paste0(ticker, ": Rolling Style Exposures (Betas)")) +
    xlab("Year") +
    ylab("Beta") +
    theme_minimal() +
    theme(
  ...
  # Directly print the plot (no saving)
  print(p)
}
...
##### Plot 7-factor contribution for each fund correctly with full factors #####

# Fund list
fund_contributions <- list(
...
# Full factor list
all_factors <- c("SmallValue", "SmallNeutral", "SmallGrowth",
                 "BigValue", "BigNeutral", "BigGrowth", "Bond")

for (fund in names(fund_contributions)) {
  df_contrib <- fund_contributions[[fund]]
  
  # Always make sure there is a Year column first
  if (!"Year" %in% colnames(df_contrib)) {
    df_contrib <- df_contrib %>%
      mutate(Year = as.numeric(substr(Period, 1, 4)))
  }
  
  if (fund %in% c("PRSVX", "VWINX")) {
    df_plot <- df_contrib %>%
      ...
      mutate(Factor = case_when(
        Factor == "SmallValue_pct" ~ "SmallValue",
        Factor == "SmallNeutral_pct" ~ "SmallNeutral",
        Factor == "SmallGrowth_pct" ~ "SmallGrowth",
        Factor == "BigValue_pct" ~ "BigValue",
        Factor == "BigNeutral_pct" ~ "BigNeutral",
        Factor == "BigGrowth_pct" ~ "BigGrowth",
        Factor == "Bond_pct" ~ "Bond",
        TRUE ~ Factor
      ))
    ...
    # Add missing factors (SmallValue, SmallNeutral, etc.) at 0
    missing_factors <- setdiff(all_factors, unique(df_plot$Factor))
    for (mf in missing_factors) {
      df_plot <- bind_rows(df_plot, df_contrib %>%
                             select(Year) %>%
                             mutate(Factor = mf, Contribution = 0))
    }
  }
  
  # Set factor order
  df_plot$Factor <- factor(df_plot$Factor, levels = c(
  ...
  # Plot
  p_contrib <- ggplot(df_plot, aes(x = Year, y = Contribution, color = Factor)) +
    geom_line(size = 1.2) +
    geom_point(size = 2) +
